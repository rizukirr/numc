...
typedef void (*NumcBinaryReductionKernel)(const char *a, const char *b,
...
#define DEFINE_BINARY_REDUCTION_KERNEL(OP_NAME, TYPE_ENUM, C_TYPE, INIT, EXPR, \
                                       OMP_OP)                                 \
  static void _kern_##OP_NAME##_##TYPE_ENUM(const char *a, const char *b,      \
    ...
    if (sa == es && sb == es) {                                                \
      ...
      for (size_t i = 0; i < n; i++) {                                         \
        C_TYPE val_a = *(const C_TYPE *)(a + i * sa);                          \
        C_TYPE val_b = *(const C_TYPE *)(b + i * sb);                          \
        acc = (EXPR);                                                          \
      }                                                                        \
    }                                                                          \
    *(C_TYPE *)out = acc;                                                      \
  ...
  static void _kern_##OP_NAME##_##TYPE_ENUM(const char *a, char *out,          \
    ...
    if (sa == (intptr_t)sizeof(C_TYPE)) {                                      \
      ...
      for (size_t base = 0; base < n; base += NUMC_ARGCHUNK) {                 \
        size_t len = (base + NUMC_ARGCHUNK <= n) ? NUMC_ARGCHUNK : (n - base); \
