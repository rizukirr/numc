...
#define FAST_CMP_SINT_AVX2(SFX, CT, VPV, CMPEQ, CMPGT, SET1)              \
  static inline void _fast_eq_##SFX##_avx2(const void *restrict ap,       \
  ...
  static inline void _fast_gt_##SFX##_avx2(const void *restrict ap,       \
                                           const void *restrict bp,       \
                                           void *restrict op, size_t n) { \
    const CT *a = (const CT *)ap;                                         \
    const CT *b = (const CT *)bp;                                         \
  ...
  static inline void _fast_lt_##SFX##_avx2(const void *restrict ap,       \
                                           const void *restrict bp,       \
                                           void *restrict op, size_t n) { \
    _fast_gt_##SFX##_avx2(bp, ap, op, n);                                 \
  }                                                                       \
  static inline void _fast_ge_##SFX##_avx2(const void *restrict ap,       \
                                           const void *restrict bp,       \
                                           void *restrict op, size_t n) { \
    const CT *a = (const CT *)ap;                                         \
    const CT *b = (const CT *)bp;                                         \
  ...
  static inline void _fast_le_##SFX##_avx2(const void *restrict ap,       \
                                           const void *restrict bp,       \
                                           void *restrict op, size_t n) { \
    _fast_ge_##SFX##_avx2(bp, ap, op, n);                                 \
  }

FAST_CMP_SINT_AVX2(i8, int8_t, 32, _mm256_cmpeq_epi8, _mm256_cmpgt_epi8,
                   _mm256_set1_epi8)
FAST_CMP_SINT_AVX2(i16, int16_t, 16, _mm256_cmpeq_epi16, _mm256_cmpgt_epi16,
                   _mm256_set1_epi16)
FAST_CMP_SINT_AVX2(i32, int32_t, 8, _mm256_cmpeq_epi32, _mm256_cmpgt_epi32,
                   _mm256_set1_epi32)
FAST_CMP_SINT_AVX2(i64, int64_t, 4, _mm256_cmpeq_epi64, _mm256_cmpgt_epi64,
                   _mm256_set1_epi64x)
...
FAST_CMP_UINT_AVX2(u8, uint8_t, 32, _mm256_cmpeq_epi8, _mm256_cmpgt_epi8,
                   _mm256_set1_epi8, _mm256_set1_epi8((char)0x80))
FAST_CMP_UINT_AVX2(u16, uint16_t, 16, _mm256_cmpeq_epi16, _mm256_cmpgt_epi16,
                   _mm256_set1_epi16, _mm256_set1_epi16((short)0x8000))
FAST_CMP_UINT_AVX2(u32, uint32_t, 8, _mm256_cmpeq_epi32, _mm256_cmpgt_epi32,
                   _mm256_set1_epi32, _mm256_set1_epi32((int)0x80000000))
FAST_CMP_UINT_AVX2(u64, uint64_t, 4, _mm256_cmpeq_epi64, _mm256_cmpgt_epi64,
                   _mm256_set1_epi64x,
  ...
  static inline void _fast_##OP##_f32_avx2(const void *restrict ap,       \
    ...
    for (; i + 8 <= n; i += 8) {                                          \
      ...
      _mm256_storeu_ps(out + i,                                           \
                       _mm256_and_ps(_mm256_cmp_ps(va, vb, PRED), one));  \
    }                                                                     \
    for (; i < n; i++)                                                    \
  ...
  static inline void _fast_##OP##_f64_avx2(const void *restrict ap,       \
    ...
    for (; i + 4 <= n; i += 4) {                                          \
      ...
      _mm256_storeu_pd(out + i,                                           \
                       _mm256_and_pd(_mm256_cmp_pd(va, vb, PRED), one));  \
    }                                                                     \
    for (; i < n; i++)                                                    \
