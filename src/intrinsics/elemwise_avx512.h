...
#define FAST_BIN_INT_AVX512(OP, SFX, CT, VPV, VEC_OP, TAIL_EXPR)           \
  static inline void _fast_##OP##_##SFX##_avx512(                          \
      const void *restrict ap, const void *restrict bp, void *restrict op, \
      size_t n) {                                                          \
...
FAST_BIN_INT_AVX512(add, i32, int32_t, 16, _mm512_add_epi32, a[i] + b[i])
FAST_BIN_INT_AVX512(add, i64, int64_t, 8, _mm512_add_epi64, a[i] + b[i])
FAST_BIN_INT_AVX512(add, u8, uint8_t, 64, _mm512_add_epi8, a[i] + b[i])
FAST_BIN_INT_AVX512(add, u16, uint16_t, 32, _mm512_add_epi16, a[i] + b[i])
FAST_BIN_INT_AVX512(add, u32, uint32_t, 16, _mm512_add_epi32, a[i] + b[i])
FAST_BIN_INT_AVX512(add, u64, uint64_t, 8, _mm512_add_epi64, a[i] + b[i])
...
FAST_BIN_INT_AVX512(sub, i32, int32_t, 16, _mm512_sub_epi32, a[i] - b[i])
FAST_BIN_INT_AVX512(sub, i64, int64_t, 8, _mm512_sub_epi64, a[i] - b[i])
FAST_BIN_INT_AVX512(sub, u8, uint8_t, 64, _mm512_sub_epi8, a[i] - b[i])
FAST_BIN_INT_AVX512(sub, u16, uint16_t, 32, _mm512_sub_epi16, a[i] - b[i])
FAST_BIN_INT_AVX512(sub, u32, uint32_t, 16, _mm512_sub_epi32, a[i] - b[i])
FAST_BIN_INT_AVX512(sub, u64, uint64_t, 8, _mm512_sub_epi64, a[i] - b[i])
...
static inline void _fast_mul_i8_avx512(const void *restrict ap,
  ...
  for (; i + 64 <= n; i += 64) {
    ...
    __m512i hi = _mm512_slli_epi16(
        _mm512_mullo_epi16(_mm512_srli_epi16(va, 8), _mm512_srli_epi16(vb, 8)),
        8);
...
static inline void _fast_neg_f32_avx512(const void *restrict ap,
  ...
  size_t i = 0;
  for (; i + 16 <= n; i += 16)
    _mm512_storeu_ps(out + i, _mm512_xor_ps(_mm512_loadu_ps(a + i), sign));
  for (; i < n; i++)
    out[i] = -a[i];
}
...
static inline void _fast_neg_f64_avx512(const void *restrict ap,
  ...
  size_t i = 0;
  for (; i + 8 <= n; i += 8)
    _mm512_storeu_pd(out + i, _mm512_xor_pd(_mm512_loadu_pd(a + i), sign));
  for (; i < n; i++)
    out[i] = -a[i];
}
...
static inline void _fast_abs_f32_avx512(const void *restrict ap,
  ...
  size_t i = 0;
  for (; i + 16 <= n; i += 16)
    _mm512_storeu_ps(out + i, _mm512_and_ps(_mm512_loadu_ps(a + i), mask));
  for (; i < n; i++)
    out[i] = a[i] < 0 ? -a[i] : a[i];
}
...
static inline void _fast_abs_f64_avx512(const void *restrict ap,
  ...
  size_t i = 0;
  for (; i + 8 <= n; i += 8)
    _mm512_storeu_pd(out + i, _mm512_and_pd(_mm512_loadu_pd(a + i), mask));
  for (; i < n; i++)
    out[i] = a[i] < 0 ? -a[i] : a[i];
}
