 ...
 *
 * SoA layout (_prng_s[word][lane]):
 *   _prng_s[0][0..3]  = s0 for streams 0-3
 *   _prng_s[1][0..3]  = s1 for streams 0-3
 *   _prng_s[2][0..3]  = s2 for streams 0-3
 *   _prng_s[3][0..3]  = s3 for streams 0-3
 *
...
static _Thread_local uint64_t _prng_s[4][NUMC_PRNG_LANES];
static _Thread_local bool _prng_seeded = false;
...
static inline void _prng_seed(uint64_t seed) {
...
static inline void _prng_ensure_seeded(void) {
  if (!_prng_seeded)
    _prng_seed(UINT64_C(0xdeadbeefcafe1234));
}
 ...
 *
 * Caller loads state from _prng_s into locals before the loop and
...
#define XOSHIRO_STEP_4(s0, s1, s2, s3, out)                                   \
  do {                                                                         \
    ...
    s2[0] ^= s0[0]; s2[1] ^= s0[1]; s2[2] ^= s0[2]; s2[3] ^= s0[3];          \
    s3[0] ^= s1[0]; s3[1] ^= s1[1]; s3[2] ^= s1[2]; s3[3] ^= s1[3];          \
    s1[0] ^= s2[0]; s1[1] ^= s2[1]; s1[2] ^= s2[2]; s1[3] ^= s2[3];          \
    s0[0] ^= s3[0]; s0[1] ^= s3[1]; s0[2] ^= s3[2]; s0[3] ^= s3[3];          \
    s2[0] ^= _t0;   s2[1] ^= _t1;   s2[2] ^= _t2;   s2[3] ^= _t3;            \
    s3[0] = _rotl64(s3[0], 45); s3[1] = _rotl64(s3[1], 45);                   \
    s3[2] = _rotl64(s3[2], 45); s3[3] = _rotl64(s3[3], 45);                   \
    (out)[0] = _r0; (out)[1] = _r1;                                            \
    (out)[2] = _r2; (out)[3] = _r3;                                            \
  } while (0)
...
static inline uint64_t _prng_next_scalar(void) {
  _prng_ensure_seeded();
  /* use lane 0 only */
  const uint64_t result = _rotl64(_prng_s[1][0] * 5, 7) * 9;
  const uint64_t t = _prng_s[1][0] << 17;
  _prng_s[2][0] ^= _prng_s[0][0];
  _prng_s[3][0] ^= _prng_s[1][0];
  _prng_s[1][0] ^= _prng_s[2][0];
  _prng_s[0][0] ^= _prng_s[3][0];
  _prng_s[2][0] ^= t;
  _prng_s[3][0] = _rotl64(_prng_s[3][0], 45);
  return result;
}
...
static _Thread_local double _bm_spare;
static _Thread_local bool _bm_spare_ready = false;

static inline double _prng_normal_f64(void) {
  if (_bm_spare_ready) {
    _bm_spare_ready = false;
    return _bm_spare;
  }
  double u1;
  do { u1 = _prng_f64(); } while (u1 == 0.0);
  double u2 = _prng_f64();
  const double mag = sqrt(-2.0 * _log_f64(u1));
  const double two_pi_u2 = 6.283185307179586476925 * u2;
  _bm_spare = mag * _sin_f64(two_pi_u2);
  _bm_spare_ready = true;
...
static inline void _prng_skip(uint64_t s[4], size_t skip) {
 ...
 *   global SoA by (tid * chunk) xoshiro steps via _prng_skip(), then
 *   runs the same 4-wide SIMD loop on its slice.
 *   _prng_get_tid() wraps omp_get_thread_num() safely without #ifdef
...
#define DEFINE_RAND_KERNEL(TYPE_ENUM, C_TYPE, CONVERT_EXPR)                   \
  static void _kern_rand_##TYPE_ENUM(char *out, size_t n) {                   \
    ...
    if (total_bytes <= NUMC_OMP_BYTE_THRESHOLD) {                              \
      ...
      memcpy(s0, _prng_s[0], sizeof s0);                                      \
      memcpy(s1, _prng_s[1], sizeof s1);                                      \
      memcpy(s2, _prng_s[2], sizeof s2);                                      \
      memcpy(s3, _prng_s[3], sizeof s3);                                      \
      size_t i = 0;                                                            \
      uint64_t raw[4];                                                         \
      ...
      memcpy(_prng_s[0], s0, sizeof s0);                                      \
      memcpy(_prng_s[1], s1, sizeof s1);                                      \
      memcpy(_prng_s[2], s2, sizeof s2);                                      \
      memcpy(_prng_s[3], s3, sizeof s3);                                      \
      for (; i < n; i++)                                                      \
      ...
      uint64_t _base[4] = {                                                   \
          _prng_s[0][0], _prng_s[1][0],                                       \
          _prng_s[2][0], _prng_s[3][0],                                       \
      };                                                                      \
      size_t _chunk = (n + (size_t)_nthreads - 1) / (size_t)_nthreads;       \
      NUMC_PRAGMA(omp parallel num_threads(_nthreads))                        \
      {                                                                       \
        int _tid = _prng_get_tid();                                           \
        size_t _start = (size_t)_tid * _chunk;                                \
        size_t _end   = _start + _chunk < n ? _start + _chunk : n;           \
        uint64_t ts[4] = {_base[0], _base[1], _base[2], _base[3]};           \
        _prng_skip(ts, _start);                                               \
        uint64_t s0[4], s1[4], s2[4], s3[4];                                  \
        for (int _l = 0; _l < 4; _l++) {                                      \
          uint64_t _lt[4] = {ts[0], ts[1], ts[2], ts[3]};                     \
          _prng_skip(_lt, (size_t)_l);                                        \
          s0[_l] = _lt[0]; s1[_l] = _lt[1];                                  \
          s2[_l] = _lt[2]; s3[_l] = _lt[3];                                  \
        }                                                                     \
        size_t _i = _start;                                                   \
        uint64_t _raw[4];                                                     \
        for (; _i + NUMC_PRNG_LANES <= _end; _i += NUMC_PRNG_LANES) {        \
        ...
        for (; _i < _end; _i++) {                                              \
          ...
          _ts[2] ^= _ts[0]; _ts[3] ^= _ts[1];                                 \
          _ts[1] ^= _ts[2]; _ts[0] ^= _ts[3];                                 \
          _ts[2] ^= _t;                                                       \
      ...
      _prng_skip(_base, n);                                                   \
      _prng_s[0][0] = _base[0]; _prng_s[1][0] = _base[1];                    \
      _prng_s[2][0] = _base[2]; _prng_s[3][0] = _base[3];                    \
    }                                                                         \
  }
