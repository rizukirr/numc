...
static _Thread_local char error_buf[NUMC_ERROR_MSG_SIZE];
static _Thread_local NumcError error = {0, NULL};
...
int numc_set_error_v(int code, const char *func, const char *file, int line,
  ...
  snprintf(error_buf, NUMC_ERROR_MSG_SIZE, "%s@%s:%d: %s",
           func ? func : "(unknown)", file ? file : "(unknown)", line,
           formatted);
#else
  /* compact stored form: func: message */
  snprintf(error_buf, NUMC_ERROR_MSG_SIZE, "%s: %s", func ? func : "(unknown)",
           formatted);
#endif
