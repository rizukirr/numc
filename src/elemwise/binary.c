...
/* BLIS Level 1v (copy+addv, copy+scalv, etc.) requires two passes over the
 * data, doubling memory traffic for bandwidth-bound element-wise ops. The
 * single-pass vectorized kernels (NUMC_OMP_FOR + SIMD) are faster for
 * add, sub, and scalar operations. BLIS is only used where it provides
 * true fused kernels: dot, gemm, etc. */
