...
void test_array_offset_2d(void) {
  ...
  assert(array_offset(arr, (size_t[]){1, 0}) == 16);
...
void test_array_offset_3d(void) {
  ...
  // [0,1,0] -> 0*48 + 1*16 = 16
  assert(array_offset(arr, (size_t[]){0, 1, 0}) == 16);
...
void test_array_bounds_check_invalid(void) {
  ...
  assert(array_bounds_check(arr, (size_t[]){10, 10}) == -1); // far out of bounds
...
void test_array_get_basic(void) {
  ...
  // Test with int array
  Array *arr_int = array_create(1, (size_t[]){5}, DTYPE_INT, NULL);
  ...
  Array *arr_float = array_create(1, (size_t[]){3}, DTYPE_FLOAT, NULL);
...
void test_array_free_basic(void) {
  ...
  Array *arr = array_create(2, (size_t[]){10, 10}, DTYPE_INT, NULL);
  assert(arr != NULL);
...
void test_array_free_ownership(void) {
  ...
  Array *arr1 = array_create(1, (size_t[]){5}, DTYPE_INT, NULL);
  assert(arr1->owns_data == 1);
  array_free(arr1);
  
  // Create slice (doesn't own data)
  Array *base = array_create(1, (size_t[]){10}, DTYPE_INT, NULL);
...
void test_type_safe_accessors(void) {
  ...
  // Test array_geti
  Array *arr_int = array_create(1, (size_t[]){3}, DTYPE_INT, NULL);
  int *data_int = (int *)arr_int->data;
  data_int[0] = 10;
  data_int[1] = 20;
  data_int[2] = 30;
  assert(*array_geti(arr_int, (size_t[]){0}) == 10);
  assert(*array_geti(arr_int, (size_t[]){2}) == 30);
  array_free(arr_int);
  
  // Test array_getf
  Array *arr_float = array_create(1, (size_t[]){2}, DTYPE_FLOAT, NULL);
  float *data_float = (float *)arr_float->data;
  data_float[0] = 1.5f;
  data_float[1] = 2.5f;
  assert(*array_getf(arr_float, (size_t[]){0}) == 1.5f);
  assert(*array_getf(arr_float, (size_t[]){1}) == 2.5f);
  array_free(arr_float);
  
  // Test array_getui
  Array *arr_uint = array_create(1, (size_t[]){2}, DTYPE_UINT, NULL);
  unsigned int *data_uint = (unsigned int *)arr_uint->data;
  data_uint[0] = 100;
  data_uint[1] = 200;
  assert(*array_getui(arr_uint, (size_t[]){0}) == 100);
  assert(*array_getui(arr_uint, (size_t[]){1}) == 200);
  array_free(arr_uint);
  
  // Test array_getl
  Array *arr_long = array_create(1, (size_t[]){2}, DTYPE_LONG, NULL);
  long *data_long = (long *)arr_long->data;
  data_long[0] = 1000L;
  data_long[1] = 2000L;
  assert(*array_getl(arr_long, (size_t[]){0}) == 1000L);
  assert(*array_getl(arr_long, (size_t[]){1}) == 2000L);
  array_free(arr_long);
  
  // Test array_getul
  Array *arr_ulong = array_create(1, (size_t[]){2}, DTYPE_ULONG, NULL);
  unsigned long *data_ulong = (unsigned long *)arr_ulong->data;
  data_ulong[0] = 10000UL;
  data_ulong[1] = 20000UL;
  assert(*array_getul(arr_ulong, (size_t[]){0}) == 10000UL);
  assert(*array_getul(arr_ulong, (size_t[]){1}) == 20000UL);
  array_free(arr_ulong);
  
  // Test array_gets
  Array *arr_short = array_create(1, (size_t[]){2}, DTYPE_SHORT, NULL);
  short *data_short = (short *)arr_short->data;
  data_short[0] = 100;
  data_short[1] = 200;
  assert(*array_gets(arr_short, (size_t[]){0}) == 100);
  assert(*array_gets(arr_short, (size_t[]){1}) == 200);
  array_free(arr_short);
  
  // Test array_getus
  Array *arr_ushort = array_create(1, (size_t[]){2}, DTYPE_USHORT, NULL);
  unsigned short *data_ushort = (unsigned short *)arr_ushort->data;
  data_ushort[0] = 300;
  data_ushort[1] = 400;
  assert(*array_getus(arr_ushort, (size_t[]){0}) == 300);
  assert(*array_getus(arr_ushort, (size_t[]){1}) == 400);
  array_free(arr_ushort);
  
  // Test array_getb
  Array *arr_byte = array_create(1, (size_t[]){2}, DTYPE_BYTE, NULL);
  signed char *data_byte = (signed char *)arr_byte->data;
  data_byte[0] = -10;
  data_byte[1] = 20;
  assert(*array_getb(arr_byte, (size_t[]){0}) == -10);
  assert(*array_getb(arr_byte, (size_t[]){1}) == 20);
  array_free(arr_byte);
  
  // Test array_getub
  Array *arr_ubyte = array_create(1, (size_t[]){2}, DTYPE_UBYTE, NULL);
  unsigned char *data_ubyte = (unsigned char *)arr_ubyte->data;
  data_ubyte[0] = 50;
  data_ubyte[1] = 100;
  assert(*array_getub(arr_ubyte, (size_t[]){0}) == 50);
  assert(*array_getub(arr_ubyte, (size_t[]){1}) == 100);
