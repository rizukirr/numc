...
static int test_max_type_mismatch(void) {
  ...
  ASSERT_MSG(err == NUMC_ERR_TYPE,
             "max type mismatch should return NUMC_ERR_TYPE");
...
static int test_max_out_not_scalar(void) {
  ...
  ASSERT_MSG(err == NUMC_ERR_SHAPE,
             "max with non-scalar out should return NUMC_ERR_SHAPE");
...
static int test_max_axis_null(void) {
  ...
  ASSERT_MSG(err == NUMC_ERR_NULL,
             "max_axis(NULL) should return NUMC_ERR_NULL");
  return 0;
}
...
static int test_max_axis_shape_mismatch(void) {
  ...
  int err = numc_max_axis(a, 0, 0, out);
  ASSERT_MSG(err == NUMC_ERR_SHAPE,
             "max_axis with wrong output shape should fail");
