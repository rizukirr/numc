...
void test_concat_axis0_1d(void) {
  ...
  // Create [1, 2, 3]
  Array *a = array_create(1, (size_t[]){3}, DTYPE_INT, NULL);
  ...
  // Create [4, 5]
  Array *b = array_create(1, (size_t[]){2}, DTYPE_INT, NULL);
  int *data_b = (int *)b->data;
  data_b[0] = 4;
  data_b[1] = 5;

  // Concatenate: should get [1, 2, 3, 4, 5]
  Array *result = array_concat(a, b, 0);
  assert(result != NULL);
...
void test_concat_axis0_2d(void) {
  ...
  Array *a = array_create(2, (size_t[]){2, 3}, DTYPE_INT, NULL);
  int *data_a = (int *)a->data;
  for (int i = 0; i < 6; i++) {
    data_a[i] = i + 1;
  }
  ...
  Array *b = array_create(2, (size_t[]){1, 3}, DTYPE_INT, NULL);
  int *data_b = (int *)b->data;
  for (int i = 0; i < 3; i++) {
    data_b[i] = i + 7;
  }
...
void test_concat_axis1_2d(void) {
  ...
  Array *a = array_create(2, (size_t[]){2, 2}, DTYPE_INT, NULL);
  int *data_a = (int *)a->data;
  data_a[0] = 1;
  data_a[1] = 2;
  data_a[2] = 3;
  ...
  Array *b = array_create(2, (size_t[]){2, 1}, DTYPE_INT, NULL);
  int *data_b = (int *)b->data;
  data_b[0] = 5;
  data_b[1] = 6;

  // Concatenate along axis 1: should get [[1, 2, 5], [3, 4, 6]] - shape [2, 3]
  Array *result = array_concat(a, b, 1);
  assert(result != NULL);
...
void test_concat_float_arrays(void) {
  ...
  // Create [1.0, 2.0, 3.0]
  Array *a = array_create(1, (size_t[]){3}, DTYPE_FLOAT, NULL);
  ...
  // Create [4.0, 5.0]
  Array *b = array_create(1, (size_t[]){2}, DTYPE_FLOAT, NULL);
