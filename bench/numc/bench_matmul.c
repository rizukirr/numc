...
static double run(NumcArray *a, NumcArray *b, NumcArray *out, int warmup,
                  int iters) {
...
static void bench_square_scaling(void) {
  ...
  static const struct {
    ...
    int iters;
      ...
      {32, 100, 500}, {64, 50, 200}, {128, 50, 50}, {256, 50, 20}, {512, 10, 5},
  };
  ...
  for (int s = 0; s < nsizes; s++) {
    ...
    char val[8];
    fill_scalar(NUMC_DTYPE_FLOAT32, val);
...
static void bench_shape_variants(void) {
  ...
  for (int s = 0; s < nshapes; s++) {
    ...
    printf("  %-24s  %8.2f  %8.3f  %8.0f K\n", shapes[s].label, us,
           gflops(M, K, N, us), total_flops / 1e3);
    numc_ctx_free(ctx);
  }
}
