/**
 * @file benchmark_add.c
 * @brief Focused benchmark for ADD operation only
 */
...
double benchmark_add(const Array *a, const Array *b, Array *out, int iterations) {
...
void benchmark_type(NUMC_TYPE numc_type, const char *type_name, size_t size) {
  ...
  printf("  %-10s Add: %8.2f μs  (%10.2f Mops/sec)\n",
         type_name, avg_add, size / avg_add);
