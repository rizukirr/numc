/**
 * @file benchmark_sum.c
 * @brief Focused benchmark for SUM reduction operation
 */
...
double benchmark_sum(const Array *a, void *result, int iterations) {
...
void benchmark_type(NUMC_TYPE numc_type, const char *type_name, size_t size) {
  ...
  // Benchmark
  double sum_time = benchmark_sum(a, result, BENCHMARK_ITERATIONS);
  double avg_sum = (sum_time / BENCHMARK_ITERATIONS) * 1000000;

  printf("  %-10s Sum: %8.2f μs  (%10.2f Mops/sec)\n",
         type_name, avg_sum, size / avg_sum);
...
int main(void) {
  printf("╔═══════════════════════════════════════════════════════════════════╗\n");
  printf("║             SUM Reduction Benchmark (1M elements)                ║\n");
  printf("╚═══════════════════════════════════════════════════════════════════╝\n");
  printf("\n");
