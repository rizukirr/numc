...
double benchmark_binary_op(int (*op_func)(const Array *, const Array *, Array *),
...
void benchmark_all_operations(NUMC_TYPE numc_type, const char *type_name, size_t size) {
  printf("\n%s Array (%zu elements):\n", type_name, size);
  ...
  // Addition
  double add_time = benchmark_binary_op(array_add, a, b, out, BENCHMARK_ITERATIONS);
  double avg_add = (add_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Add:         %10.2f μs  (%10.2f Mops/sec)\n", avg_add, size / avg_add);
  
  // Subtraction
  double sub_time = benchmark_binary_op(array_subtract, a, b, out, BENCHMARK_ITERATIONS);
  double avg_sub = (sub_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Subtract:    %10.2f μs  (%10.2f Mops/sec)\n", avg_sub, size / avg_sub);
  
  // Multiplication
  double mul_time = benchmark_binary_op(array_multiply, a, b, out, BENCHMARK_ITERATIONS);
  double avg_mul = (mul_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Multiply:    %10.2f μs  (%10.2f Mops/sec)\n", avg_mul, size / avg_mul);
  
  // Division
  double div_time = benchmark_binary_op(array_divide, a, b, out, BENCHMARK_ITERATIONS);
  double avg_div = (div_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Divide:      %10.2f μs  (%10.2f Mops/sec)\n", avg_div, size / avg_div);
  
  printf("\n  Scalar Operations:\n");
  
  // Add scalar
  double adds_time = benchmark_scalar_op(array_add_scalar, a, scalar, out, BENCHMARK_ITERATIONS);
  double avg_adds = (adds_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Add scalar:  %10.2f μs  (%10.2f Mops/sec)\n", avg_adds, size / avg_adds);
  
  // Subtract scalar
  double subs_time = benchmark_scalar_op(array_subtract_scalar, a, scalar, out, BENCHMARK_ITERATIONS);
  double avg_subs = (subs_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Sub scalar:  %10.2f μs  (%10.2f Mops/sec)\n", avg_subs, size / avg_subs);
  
  // Multiply scalar
  double muls_time = benchmark_scalar_op(array_multiply_scalar, a, scalar, out, BENCHMARK_ITERATIONS);
  double avg_muls = (muls_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Mul scalar:  %10.2f μs  (%10.2f Mops/sec)\n", avg_muls, size / avg_muls);
  
  // Divide scalar
  double divs_time = benchmark_scalar_op(array_divide_scalar, a, scalar, out, BENCHMARK_ITERATIONS);
  double avg_divs = (divs_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Div scalar:  %10.2f μs  (%10.2f Mops/sec)\n", avg_divs, size / avg_divs);
  
  printf("\n  Reduction Operations:\n");
  
  // Sum
  double sum_time = benchmark_reduce_op(array_sum, a, result, BENCHMARK_ITERATIONS);
  double avg_sum = (sum_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Sum:         %10.2f μs  (%10.2f Mops/sec)\n", avg_sum, size / avg_sum);
  
  // Min
  double min_time = benchmark_reduce_op(array_min, a, result, BENCHMARK_ITERATIONS);
  double avg_min = (min_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Min:         %10.2f μs  (%10.2f Mops/sec)\n", avg_min, size / avg_min);
  
  // Max
  double max_time = benchmark_reduce_op(array_max, a, result, BENCHMARK_ITERATIONS);
  double avg_max = (max_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Max:         %10.2f μs  (%10.2f Mops/sec)\n", avg_max, size / avg_max);
  
  // Dot product
  double dot_time = benchmark_dot_op(array_dot, a, b, result, BENCHMARK_ITERATIONS);
  double avg_dot = (dot_time / BENCHMARK_ITERATIONS) * 1000000;
  printf("    Dot product: %10.2f μs  (%10.2f Mops/sec)\n", avg_dot, size / avg_dot);
...
int main(void) {
  ...
  printf("  Scalar:     add_scalar, subtract_scalar, multiply_scalar, divide_scalar\n");
  ...
  printf("  - 10 data types (BYTE, UBYTE, SHORT, USHORT, INT32, UINT32, INT64, UINT64, FLOAT, DOUBLE)\n");
