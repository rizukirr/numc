/**
 * @file benchmark_divide.c
 * @brief Focused benchmark for DIVIDE operation only
 */
...
double benchmark_divide(const Array *a, const Array *b, Array *out, int iterations) {
  double start = get_time_ms();
  for (int i = 0; i < iterations; i++) {
    array_divide(a, b, out);
  }
...
void benchmark_type(NUMC_TYPE numc_type, const char *type_name, size_t size) {
  ...
  // Benchmark
  double div_time = benchmark_divide(a, b, out, BENCHMARK_ITERATIONS);
  double avg_div = (div_time / BENCHMARK_ITERATIONS) * 1000000;

  printf("  %-10s Divide: %8.2f μs  (%10.2f Mops/sec)\n",
         type_name, avg_div, size / avg_div);
...
int main(void) {
  printf("╔═══════════════════════════════════════════════════════════════════╗\n");
  printf("║            DIVIDE Operation Benchmark (1M elements)              ║\n");
  printf("╚═══════════════════════════════════════════════════════════════════╝\n");
  printf("\n");
