/**
 * @file benchmark_max.c
 * @brief Focused benchmark for MAX reduction operation
 */
...
double benchmark_max(const Array *a, void *result, int iterations) {
...
void benchmark_type(NUMC_TYPE numc_type, const char *type_name, size_t size) {
  ...
  // Benchmark
  double max_time = benchmark_max(a, result, BENCHMARK_ITERATIONS);
  double avg_max = (max_time / BENCHMARK_ITERATIONS) * 1000000;

  printf("  %-10s Max: %8.2f μs  (%10.2f Mops/sec)\n",
         type_name, avg_max, size / avg_max);
...
int main(void) {
  printf("╔═══════════════════════════════════════════════════════════════════╗\n");
  printf("║             MAX Reduction Benchmark (1M elements)                ║\n");
  printf("╚═══════════════════════════════════════════════════════════════════╝\n");
  printf("\n");
